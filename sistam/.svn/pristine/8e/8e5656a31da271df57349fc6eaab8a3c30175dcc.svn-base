...
/**
 * Demonstrates how to add an image to a worksheet and set that images size to a
 * specific number of millimetres irrespective of the width of the columns or
 * height of the rows. Overridden methods are provided so that the location of
 * the image - the cells row and column coordinates that define the top left
 * hand corners of the image - can be identified either in the familiar Excel
 * manner - A1 for instance - or using POI's methodology of a column and row
 * index where 0, 0 would indicate cell A1.
 *
 * The best way to make use of these techniques is to delay adding the image to
 * the sheet until all other work has been completed. That way, the sizes of all
 * rows and columns will have been adjusted - assuming that step was necessary.
 * Even though the anchors type is set to prevent the image moving or re-sizing,
 * this setting does not have any effect until the sheet is being viewed using
 * the Excel application.
 *
 * The key to the process is the ClientAnchor class. It defines methods that
 * allow us to define the location of an image by specifying the following;
 *
 *      * How far - in terms of coordinate positions - the image should be inset
 * from the left hand border of a cell. * How far - in terms of coordinate
 * positions - the image should be inset from the from the top of the cell. *
 * How far - in terms of coordinate positions - the right hand edge of the image
 * should protrude into a cell (measured from the cells left hand edge to the
 * images right hand edge). * How far - in terms of coordinate positions - the
 * bottom edge of the image should protrude into a row (measured from the cells
 * top edge to the images bottom edge). * The index of the column that contains
 * the cell whose top left hand corner should be aligned with the top left hand
 * corner of the image. * The index of the row that contains the cell whose top
 * left hand corner should be aligned with the images top left hand corner. *
 * The index of the column that contains the cell whose top left hand corner
 * should be aligned with the images bottom right hand corner * The index number
 * of the row that contains the cell whose top left hand corner should be
 * aligned with the images bottom right hand corner.
 *
 ...
 *
 * anchor.setDx1(0); anchor.setDy1(0); anchor.setDx2(0); anchor.setDy2(0);
 * anchor.setCol1(0); anchor.setRow1(0); anchor.setCol2(1); anchor.setRow2(1);
 *
 ...
 * into a single cell or is can lie across many cells and the latter four
 * methods are used to define just where the image should be positioned. They do
 * this by again by identifying where the top left and bottom right hand corners
 * of the image should be located but this time in terms of the indexes of the
 * cells in which those corners should be located. The setCol1() and setRow1()
 * methods together identify the cell that should contain the top left hand
 * corner of the image while setCol2() and setRow2() do the same for the images
 ...
 * and it will be aligned with the very top left hand corner of the cell.
 * Likewise, the bottom right hand corner of the image will be located in cell
 * B2 (1, 1) and it will again be aligned with the top left hand corner of the
 * cell. This has the effect of making the image seem to occupy the whole of
 * cell A1. Interestingly, it also has an effect on the images resizing
 * behaviour because testing has demonstrated that if the image is wholly
 * contained within one cell and is not 'attached' for want of a better word, to
 * a neighbouring cell, then that image will not increase in size in response to
 * the user dragging the column wider or the cell higher.
 *
 ...
 * is accomplished by specifying the the images bottom right hand corner should
 * be aligned with the bottom right hand corner of the cell. It is also a case
 ...
 * anchor.setCol1(0); anchor.setRow1(0); anchor.setCol2(0); anchor.setRow2(0);
 *
 ...
 * hand corner of cell A1 and that it's bottom right hand corner is also aligned
 * with the top left hand corner of cell A1. Yet, running this code would see
 * the image fully occupying cell A1. That is the result of the values passed to
 * parameters three and four; these I have referred to as determining the images
 * coordinates within the cell. They indicate that the image should occupy - in
 * order - the full width of the column and the full height of the row.
 *
 * The co-ordinate values shown are the maxima; and they are independent of row
 * height/column width and of the font used. Passing 255 will always result in
 * the image occupying the full height of the row and passing 1023 will always
 * result in the image occupying the full width of the column. They help in
 * situations where an image is larger than a column/row and must overlap into
 * the next column/row. Using them does mean, however, that it is often
 ...
 * from that required; testing has so far found this to be in the region of plus
 * or minus two millimetres. Most likely by modifying the way the calculations
 * are performed - possibly using double(s) throughout and rounding the values
 * at the correct point - it is likely that these errors could be reduced or
 * removed.
 *
 * A note concerning Excels image resizing behaviour. The ClientAnchor class
 * contains a method called setAnchorType(int) which can be used to determine
 * how Excel will resize an image in response to the user increasing or
 * decreasing the dimensions of the cell containing the image. There are three
 * values that can be passed to this method; 0 = To move and size the image with
 * the cell, 2 = To move but don't size the image with the cell, 3 = To prevent
 * the image from moving or being resized along with the cell. If an image is
 * inserted using this class and placed into a single cell then if the
 * setAnchorType(int) method is called and a value of either 0 or 2 passed to
 * it, the resultant resizing behaviour may be a surprise. The image will not
 ...
 *
 * @author Mark Beardsley [msb at apache.org] and Mark Southern [southern at
 * scripps.edu]
 * @version 1.00 5th August 2009. 2.00 26th February 2010. Ported to make use of
 * the the SS usermodel classes. Ability to reuse the Drawing Patriarch so that
 * multiple images can be inserted without unintentionally erasing earlier
 * images. Check on image type added; i.e. jpg, jpeg or png. The String used to
 * contain the files name is now converted into a URL. 2.10 17th May 2012
 * Corrected gross error that occurred when using the code with XSSF or SXSSF
 * workbooks. In short, the code did not correctly calculate the size of the
 * image(s) owing to the use of EMUs within the OOXML file format. That problem
 * has largely been corrected although it should be mentioned that images are
 * not sized with the same level of accuracy. Discrepancies of up to 2mm have
 * been noted in testing. Further investigation will continue to rectify this
 * issue.
 */
public class AddDimensionedImageToXLS {
    
    private static final Logger log = LoggerFactory.getLogger(AddDimensionedImageToXLS.class);
     ...
     * top left hand corner should be aligned with the top left hand corner of
     * the image; for example "A1", "A2" etc. This is to support the familiar
     * Excel syntax. Whilst images are are not actually inserted into cells this
     * provides a convenient method of indicating where the image should be
     ...
     * passed into the method where it was, previously, recovered from the sheet
     * in order to allow multiple pictures be inserted. If the patriarch was not
     * 'cached in this manner each time it was created any previously positioned
     * images would be simply over-written.
     * @param imageFile An instance of the URL class that encapsulates the name
     * of and path to the image that is to be 'inserted into' the sheet.
     ...
     * @param resizeBehaviour A primitive int whose value will determine how the
     * code should react if the image is larger than the cell referenced by the
     * cellNumber parameter. Four constants are provided to determine what
     * should happen; AddDimensionedImage.EXPAND_ROW
     ...
     * to the resizeBehaviour parameter.
     */
     ...
     * column on the worksheet; POI column indices are zero based. Together with
     * the rowNumber parameter's value, this parameter identifies a cell on the
     * worksheet. The images top left hand corner will be aligned with the top
     * left hand corner of this cell.
     * @param rowNumber A primitive int that contains the index number of a row
     * on the worksheet; POI row indices are zero based. Together with the
     * rowNumber parameter's value, this parameter identifies a cell on the
     * worksheet. The images top left hand corner will be aligned with the top
     * left hand corner of this cell.
     ...
     * @param resizeBehaviour A primitive int whose value will determine how the
     * code should react if the image is larger than the cell referenced by the
     * colNumber and rowNumber parameters. Four constants are provided to
     * determine what should happen; AddDimensionedImage.EXPAND_ROW
     * AddDimensionedImage.EXPAND_COLUMN
     * AddDimensionedImage.EXPAND_ROW_AND_COLUMN
     * AddDimensionedImage.OVERLAY_ROW_AND_COLUMN
     * @throws java.io.FileNotFoundException If the file containing the image
     ...
     * of image data.
     * @throws java.lang.IllegalArgumentException If an invalid value is passed
     * to the resizeBehaviour parameter or if the extension of the image file
     * indicates that it is of a type that cannot currently be added to the
     * worksheet.
     */
    public void addImageToSheet(int colNumber, int rowNumber, Sheet sheet, Drawing drawing,
        ...
        if ((resizeBehaviour != AddDimensionedImageToXLS.EXPAND_COLUMN)
                && (resizeBehaviour != AddDimensionedImageToXLS.EXPAND_ROW)
                && (resizeBehaviour != AddDimensionedImageToXLS.EXPAND_ROW_AND_COLUMN)
                && (resizeBehaviour != AddDimensionedImageToXLS.OVERLAY_ROW_AND_COLUMN)) {
     ...
     * width of the column should be adjusted if the required width of the image
     * is greater than the width of the column.
     * @return An instance of the ClientAnchorDetail class that will contain the
     * index number of the column containing the cell whose top left hand corner
     * also defines the top left hand corner of the image, the index number
     * column containing the cell whose top left hand corner also defines the
     * bottom right hand corner of the image and an inset that determines how
     * far the right hand edge of the image can protrude into the next column -
     * expressed as a specific number of coordinate positions.
     */
        ...
        if (colWidthMM < reqImageWidthMM) {
            ...
            if ((resizeBehaviour == AddDimensionedImageToXLS.EXPAND_COLUMN)
                ...
                if (sheet instanceof HSSFSheet) {
                    ...
                    pictureWidthCoordinates = (int) reqImageWidthMM * AddDimensionedImageToXLS.EMU_PER_MM;
                }
            ...
            else if ((resizeBehaviour == AddDimensionedImageToXLS.OVERLAY_ROW_AND_COLUMN)
                    || (resizeBehaviour == AddDimensionedImageToXLS.EXPAND_ROW)) {
            ...
            if (sheet instanceof HSSFSheet) {
                ...
                pictureWidthCoordinates = (int) reqImageWidthMM
                        * AddDimensionedImageToXLS.EMU_PER_MM;
            }
            colClientAnchorDetail = new ClientAnchorDetail(colNumber,
                    colNumber, pictureWidthCoordinates);
        }
     ...
     * height of the image in millimetres
     * @param resizeBehaviour A primitive int whose value will indicate how the
     * height of the row should be adjusted if the required height of the image
     * is greater than the height of the row.
     * @return An instance of the ClientAnchorDetail class that will contain the
     * index number of the row containing the cell whose top left hand corner
     * also defines the top left hand corner of the image, the index number of
     * the row containing the cell whose top left hand corner also defines the
     * bottom right hand corner of the image and an inset that determines how
     * far the bottom edge of the image can protrude into the next (lower) row -
     * expressed as a specific number of coordinate positions.
     */
    private ClientAnchorDetail fitImageToRows(Sheet sheet, int rowNumber,
        ...
        if (rowHeightMM < reqImageHeightMM) {
            if ((resizeBehaviour == AddDimensionedImageToXLS.EXPAND_ROW)
                    || (resizeBehaviour == AddDimensionedImageToXLS.EXPAND_ROW_AND_COLUMN)) {
            ...
            // the image out ver one or more rows.
                    ...
                    || (resizeBehaviour == AddDimensionedImageToXLS.EXPAND_COLUMN)) {
     ...
     * column that contains the cell whose top left hand corner should be
     * aligned with the top left hand corner of the image.
    ...
    private ClientAnchorDetail calculateColumnLocation(Sheet sheet,
        ...
        if ((int) totalWidthMM == (int) reqImageWidthMM) {
            ...
            if (sheet instanceof HSSFSheet) {
                ...
                anchorDetail = new ClientAnchorDetail(startingColumn,
                        toColumn, (int) reqImageWidthMM * AddDimensionedImageToXLS.EMU_PER_MM);
            }
        ...
        // for the ClientAnchorDetail object.
            ...
            if (sheet instanceof HSSFSheet) {
                ...
                inset = (int) overlapMM * AddDimensionedImageToXLS.EMU_PER_MM;
            }
     ...
     * that contains the cell whose top left hand corner should be aligned with
     ...
     * the row containing the cell whose top left hand corner also defines the
     * bottom right hand corner of the image and an inset that determines how
     * far the bottom edge can protrude into the next (lower) row - expressed as
     * a specific number of co-ordinate positions.
     */
    private ClientAnchorDetail calculateRowLocation(Sheet sheet,
        ...
        if ((int) totalRowHeightMM == (int) reqImageHeightMM) {
            if (sheet instanceof HSSFSheet) {
                ...
                clientAnchorDetail = new ClientAnchorDetail(startingRow, toRow,
                        (int) reqImageHeightMM * AddDimensionedImageToXLS.EMU_PER_MM);
            }
        } else {
     ...
     * HSSFWorkbook(fis); HSSFSheet sheet = workbook.getSheetAt(0);
    ...
    public static void main(String[] args) {
        ...
        try {
            if (args.length < 2) {
                log.error("Usage: AddDimensionedImage imageFile outputFile");
                return;
            }
            ...
            imageFile = args[0];
            outputFile = args[1];
            new AddDimensionedImageToXLS().addImageToSheet("B5", sheet, sheet.createDrawingPatriarch(),
                    new File(imageFile).toURI().toURL(), 100, 40,
                    AddDimensionedImageToXLS.EXPAND_ROW_AND_COLUMN);
            fos = new FileOutputStream(outputFile);
            workbook.write(fos);
        } catch (FileNotFoundException fnfEx) {
            log.error("Caught an: " + fnfEx.getClass().getName());
            log.error("Message: " + fnfEx.getMessage());
            log.error("Stacktrace follows...........");
            log.error(fnfEx.getMessage(), fnfEx);
        } catch (IOException ioEx) {
            log.error("Caught an: " + ioEx.getClass().getName());
    ...
    public class ClientAnchorDetail {
         ...
         *
         * @param fromIndex A primitive int that contains one of the coordinates
         * (row or column index) for the top left hand corner of the image.
         * @param toIndex A primitive int that contains one of the coordinates
         * (row or column index) for the bottom right hand corner of the image.
         * @param inset A primitive int that contains a value which indicates
         * how far the image should be inset from the top or the left hand edge
         * of a cell.
         */
    ...
    public static class ConvertImageUnits {
         ...
         *
         * @param widthUnits The width of the column or the height of the row in
         * Excels units.
         ...
         *
         * @param millimetres A primitive double that contains the columns width
         * or rows height in millimetres.
